...
の順。判定を使わない決定的部分（DSL・収集・L0 記録など）は Phase 0 と並行で
着手してよい（docs/05）。まずは **MVP 検証**（#26〜#29: 判定基盤 + アイデア 1 で
有効性を検証する。計画は #26 で docs/07 として確定）を最優先で進め、その結果で
フル実装を進めるか判断する。ロードマップ全体は
